...
unit PseudoRTL.Character;

interface

uses System.RTLConsts, PseudoRTL.SysUtils;
...
type
  ...
  public
    ...
    class function ToLower(C: Char): Char;
    class function ToUpper(C: Char): Char;
  end;
...
begin
  if (Index > Length(S)) or (Index < 1) then
    PseudoRTL.SysUtils.Error(Format(sArgumentOutOfRange_StringIndex, [Index, Length(S)]));
end;
...
begin
  ...
  begin
    if HighSurrogate > $3FF then
      PseudoRTL.SysUtils.Error(Format(sArgument_InvalidLowSurrogate, [Index]));
    if Index > Length(S) - 1 then
      PseudoRTL.SysUtils.Error(Format(sArgument_InvalidHighSurrogate, [Index]));
...
begin
  if (C > $10FFFF) or ((C >= $D800) and (C <= $DFFF)) then
    PseudoRTL.SysUtils.Error(sArgumentOutOfRange_InvalidUTF32);
  if C < $10000 then
    Result := Char(C)
  else
  begin
...
begin
  if not IsHighSurrogate(HighSurrogate) then
    PseudoRTL.SysUtils.Error(sArgumentOutOfRange_InvalidHighSurrogate);
  if not IsLowSurrogate(LowSurrogate) then
    PseudoRTL.SysUtils.Error(sArgumentOutOfRange_InvalidLowSurrogate);
  Result := ((Integer(HighSurrogate) - $D800) * $400) + (Integer(LowSurrogate) - $DC00) + $10000;
end;

constructor TCharacter.Create;
begin
  PseudoRTL.SysUtils.Error(Format(sNoConstruct, [ClassName]));
end;
